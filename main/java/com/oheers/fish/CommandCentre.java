                    ...
                    for (Rarity r : EvenMoreFish.fishCollection.keySet()) {
                        if (args[2].equalsIgnoreCase(r.getValue())) {
                            ...
                            for (Fish fish : EvenMoreFish.fishCollection.get(r)) {
                                ...
                                builder.event(new HoverEvent(HoverEvent.Action.SHOW_ITEM, TextComponent.fromLegacyText("点击来获得一条鱼"))); // The only element of the hover events basecomponents is the item json
